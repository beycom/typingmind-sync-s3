...
async function importFromS3() {
    ...
    try {
      ...
      const cloudData = await decryptData(new Uint8Array(s3Data.Body));
      const cloudTimestamp = getTimestamp(cloudData);
  
      logToConsole("success", "Cloud data stats:", {
        totalSize: `${cloudFileSize} bytes`,
        lastModified: cloudLastModified,
        timestamp: cloudTimestamp ? new Date(cloudTimestamp).toLocaleString() : 'Not available',
        deviceId: cloudData.metadata?.deviceId || 'Not available',
      ...
      const currentTimestamp = getTimestamp(currentData);
      
      logToConsole("info", "Current local data stats:", {
        timestamp: currentTimestamp ? new Date(currentTimestamp).toLocaleString() : 'Not available',
        deviceId: currentData.metadata?.deviceId || 'Not available',
        totalSize: `${JSON.stringify(currentData).length} bytes`,
      ...
      if (currentTimestamp > cloudTimestamp && currentTimestamp > 0) {
        ...
        queueCloudOperation("export-newer-data", backupToS3);
        
        lastImportStatus = "skipped-newer";
      ...
      if (shouldAlertUser) {
        try {
          ...
          if (backupInterval) {
            logToConsole("pause", `Clearing backupinterval ${backupInterval}`);
          ...
          if (!shouldProceed) {
            ...
            queueCloudOperation("user-cancelled-export", backupToS3);
            
            // Restart the backup interval
            startBackupInterval();
            return false;
          }
        } catch (error) {
      ...
      try {
        ...
        logToConsole("error", "Failed to import data:", importError);
        throw importError;
      }
    } catch (error) {
      ...
      // Ensure backup interval is restarted on error
  ...
  async function backupToS3() {
    ...
    try {
      ...
      // First check if there's existing cloud data and compare timestamps
      let cloudData = null;
      ...
      try {
        ...
        if (cloudFileSize > 0) {
          const decryptedCloudData = await decryptData(new Uint8Array(currentCloudData.Body));
          cloudData = decryptedCloudData;
          cloudTimestamp = getTimestamp(cloudData);
          logToConsole("info", `Cloud data timestamp: ${new Date(cloudTimestamp).toLocaleString()}`);
        }
      } catch (error) {
        ...
        cloudFileSize = 0;
        logToConsole("info", "No existing backup found in cloud");
      }
      ...
      if (cloudTimestamp > localTimestamp && cloudTimestamp > 0) {
        ...
        queueCloudOperation("import-newer-data", importFromS3);
        return false;
      }
  
      // Proceed with export since our data is newer or there is no cloud data
      ...
      // Use existing upload code (standard or multipart)
      if (dataSize > 5 * 1024 * 1024) {
        // Your existing multipart upload code
        ...
        Promise.all([
          ...
          .then(() => {
            ...
            logToConsole("info", "Export data summary", {
              ...
              deviceId: exportData.metadata.deviceId
            });
...
return timestampA > (timestampB + thresholdMs);
}
